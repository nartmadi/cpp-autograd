...
Value::Value(double data) : node(std::make_shared<Node>()) { node->data = data; }

double Value::data() const { return node->data; }

double Value::grad() const { return node->grad; }

Op Value::op() const { return node->op; }

std::size_t Value::parent_count() const { return node->parents.size(); }
...
void Value::backward() {
    ...
    for (auto it = topo.rbegin(); it != topo.rend(); ++it) {
        ...
        if (n->op == Op::Add) {
            ...
            n->parents[0]->grad += n->grad / n->parents[1]->data;
            n->parents[1]->grad -=
                n->grad * n->parents[0]->data / (n->parents[1]->data * n->parents[1]->data);
        } else if (n->op == Op::Power) {
            n->parents[0]->grad +=
                n->grad * (n->exponent * std::pow(n->parents[0]->data, n->exponent - 1));
